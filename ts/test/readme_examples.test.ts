...
//     (\`new FreeMusicSDK(...)\` / \`FreeMusicSDK.test(...)\` become
//     \`FreeMusicSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"v1_list":{"test01":{"id":"test01"}},"v1_lookup":{"test01":{"id":"test01"}},"v1_search":{"test01":{"id":"test01"}},"v2_list":{"test01":{"id":"test01"}},"v2_lookup":{"test01":{"id":"test01"}},"v2_search":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new FreeMusicSDK(...)\` and any
// \`FreeMusicSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+FreeMusicSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/FreeMusicSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
