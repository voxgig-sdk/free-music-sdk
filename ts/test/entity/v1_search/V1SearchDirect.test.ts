...
describe('V1SearchDirect', async () => {
  ...
  test('direct-load-v1_search', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-v1_search', setup.live)) return
  ...
  test('direct-list-v1_search', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-v1_search', setup.live)) return
    if (skipIfMissingIds(t, setup, ["api_key01"])) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['FREE_MUSIC_TEST_V1_SEARCH_ENTID']
