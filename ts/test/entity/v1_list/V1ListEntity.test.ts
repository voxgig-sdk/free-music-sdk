...
describe('V1ListEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list', 'load']) {
      if (maybeSkipControl(t, 'entityOp', 'v1_list.' + op, live)) return
    }
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set FREE_MUSIC_TEST_V__LIST_ENTID JSON to run live')
      return
    }
    ...
    const v1_list_ref01_list = await v1_list_ref01_ent.list(v1_list_ref01_match)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['FREE_MUSIC_TEST_V__LIST_ENTID']
  const idmapOverridden = null != idmapEnvVal && idmapEnvVal.trim().startsWith('{')

  const env = envOverride({
    'FREE_MUSIC_TEST_V__LIST_ENTID': idmap,
    'FREE_MUSIC_TEST_LIVE': 'FALSE',
    'FREE_MUSIC_TEST_EXPLAIN': 'FALSE',
  })

  idmap = env['FREE_MUSIC_TEST_V__LIST_ENTID']
