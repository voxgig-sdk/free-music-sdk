...
(0, node_test_1.describe)('V1SearchDirect', async () => {
    ...
    (0, node_test_1.test)('direct-exists', async () => {
        const sdk = new __1.FreeMusicSDK({
    ...
    (0, node_test_1.test)('direct-load-v1_search', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-v1_search', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["api_key01"]))
            return;
    ...
    (0, node_test_1.test)('direct-list-v1_search', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-v1_search', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["api_key01"]))
            return;
        const { client, calls } = setup;
...
function liveScenariosActive() { return false && process.env.FREE_MUSIC_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.FreeMusicSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
            apikey: env.FREE_MUSIC_APIKEY,
        }));
        let idmap = env['FREE_MUSIC_TEST_V1_SEARCH_ENTID'];
    ...
    const client = new __1.FreeMusicSDK({
        base: 'http://localhost:8080',
...
//# sourceMappingURL=V1SearchDirect.test.js.map