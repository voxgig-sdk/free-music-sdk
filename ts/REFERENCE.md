...
```ts
const result = await client.V1Lookup().load({ api_key: 'api_key', i: 'i' })
```
...
```ts
const results = await client.V1Search().list({ api_key: "example", s: "example" })
```
...
```ts
const result = await client.V1Search().load({ api_key: 'api_key', s: 's' })
```
