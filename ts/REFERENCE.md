...
```ts
const result = await client.V1List().load({ id: 'v1_list_id' })
```
...
```ts
const result = await client.V1Lookup().load({ id: 'v1_lookup_id' })
```
...
```ts
const result = await client.V1Search().load({ id: 'v1_search_id' })
```
