...
local FreeMusicSDK = {}
FreeMusicSDK.__index = FreeMusicSDK
...
FreeMusicSDK._make_feature = _make_feature


function FreeMusicSDK.new(options)
  local self = setmetatable({}, FreeMusicSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function FreeMusicSDK:options_map()
...
function FreeMusicSDK:get_utility()
  return Utility.copy(self._utility)
end


function FreeMusicSDK:get_root_ctx()
  return self._rootctx
end


function FreeMusicSDK:prepare(fetchargs)
...
function FreeMusicSDK:direct(fetchargs)
...
-- by passing it.
function FreeMusicSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function FreeMusicSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "FreeMusicSDK: graphql: " .. msg
    res.graphql = errors
  end
...
-- Idiomatic facade: client:V1List():list() / client:V1List():load({ id = ... })
...
-- Idiomatic facade: client:V1Lookup():list() / client:V1Lookup():load({ id = ... })
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
...
-- Idiomatic facade: client:V1Search():list() / client:V1Search():load({ id = ... })
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
...
-- Idiomatic facade: client:V2List():list() / client:V2List():load({ id = ... })
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
...
-- Idiomatic facade: client:V2Lookup():list() / client:V2Lookup():load({ id = ... })
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
...
-- Idiomatic facade: client:V2Search():list() / client:V2Search():load({ id = ... })
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
...
function FreeMusicSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = FreeMusicSDK.new(sdkopts)
  sdk.mode = "test"
