...
describe("V1LookupDirect", function()
  it("should direct-list-v1_lookup", function()
    local setup = v1_lookup_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-v1_lookup", setup.live and "live" or "unit")
  ...
  it("should direct-load-v1_lookup", function()
    local setup = v1_lookup_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-v1_lookup", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function v1_lookup_direct_setup(mockres)
