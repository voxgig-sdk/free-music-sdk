...
The Lua SDK for the FreeMusic API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:V1List(nil):list(nil, nil)
...
```lua
local result, err = client:V1List(nil):load({ id = "example_id" }, nil)
if err then error(err) end
print(result)
```
...
local result, err = client:FreeMusic(nil):load(
  { id = "test01" }, nil
)
