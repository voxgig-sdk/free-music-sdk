-- FreeMusic SDK V1List entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local V1ListEntity = {}
V1ListEntity.__index = V1ListEntity


function V1ListEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, V1ListEntity)
  self._name = "v1_list"
  self._client = client
...
function V1ListEntity:get_name()
  return self._name
end


function V1ListEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return V1ListEntity.new(self._client, opts)
end
...
function V1ListEntity:mark_deleted()
  self._deleted = true
end


function V1ListEntity:deleted()
  return true == self._deleted
end


function V1ListEntity:data_set(args)
  if args ~= nil then
...
function V1ListEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function V1ListEntity:match_set(args)
  if args ~= nil then
...
function V1ListEntity:match_get()
...
function V1ListEntity:stream(action, args, callopts)
...
function V1ListEntity:_run_op(ctx, post_done)
  local utility = self._utility
