...
local V1SearchEntity = {}
V1SearchEntity.__index = V1SearchEntity


function V1SearchEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, V1SearchEntity)
  self._name = "v1_search"
  self._client = client
...
function V1SearchEntity:get_name()
  return self._name
end


function V1SearchEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return V1SearchEntity.new(self._client, opts)
end
...
function V1SearchEntity:mark_deleted()
  self._deleted = true
end


function V1SearchEntity:deleted()
  return true == self._deleted
end


function V1SearchEntity:data_set(args)
  if args ~= nil then
...
function V1SearchEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function V1SearchEntity:match_set(args)
  if args ~= nil then
...
function V1SearchEntity:match_get()
...
function V1SearchEntity:stream(action, args, callopts)
...
function V1SearchEntity:_run_op(ctx, post_done)
  local utility = self._utility
