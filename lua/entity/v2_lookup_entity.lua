...
local V2LookupEntity = {}
V2LookupEntity.__index = V2LookupEntity


function V2LookupEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, V2LookupEntity)
  self._name = "v2_lookup"
  self._client = client
...
function V2LookupEntity:get_name()
  return self._name
end


function V2LookupEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return V2LookupEntity.new(self._client, opts)
end
...
function V2LookupEntity:mark_deleted()
  self._deleted = true
end


function V2LookupEntity:deleted()
  return true == self._deleted
end


function V2LookupEntity:data_set(args)
  if args ~= nil then
...
function V2LookupEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function V2LookupEntity:match_set(args)
  if args ~= nil then
...
function V2LookupEntity:match_get()
...
function V2LookupEntity:stream(action, args, callopts)
...
function V2LookupEntity:_run_op(ctx, post_done)
  local utility = self._utility
