...
local V2ListEntity = {}
V2ListEntity.__index = V2ListEntity


function V2ListEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, V2ListEntity)
  self._name = "v2_list"
  self._client = client
...
function V2ListEntity:get_name()
  return self._name
end


function V2ListEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return V2ListEntity.new(self._client, opts)
end
...
function V2ListEntity:mark_deleted()
  self._deleted = true
end


function V2ListEntity:deleted()
  return true == self._deleted
end


function V2ListEntity:data_set(args)
  if args ~= nil then
...
function V2ListEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function V2ListEntity:match_set(args)
  if args ~= nil then
...
function V2ListEntity:match_get()
...
function V2ListEntity:stream(action, args, callopts)
...
function V2ListEntity:_run_op(ctx, post_done)
  local utility = self._utility
