...
local V1LookupEntity = {}
V1LookupEntity.__index = V1LookupEntity


function V1LookupEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, V1LookupEntity)
  self._name = "v1_lookup"
  self._client = client
...
function V1LookupEntity:get_name()
  return self._name
end


function V1LookupEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return V1LookupEntity.new(self._client, opts)
end
...
function V1LookupEntity:mark_deleted()
  self._deleted = true
end


function V1LookupEntity:deleted()
  return true == self._deleted
end


function V1LookupEntity:data_set(args)
  if args ~= nil then
...
function V1LookupEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function V1LookupEntity:match_set(args)
  if args ~= nil then
...
function V1LookupEntity:match_get()
...
function V1LookupEntity:stream(action, args, callopts)
...
---@param reqmatch V1LookupLoadMatch
---@param ctrl? table
---@return V1Lookup
---@return string? err
function V1LookupEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function V1LookupEntity:_run_op(ctx, post_done)
  local utility = self._utility
