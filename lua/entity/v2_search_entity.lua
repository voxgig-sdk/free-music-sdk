...
local V2SearchEntity = {}
V2SearchEntity.__index = V2SearchEntity


function V2SearchEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, V2SearchEntity)
  self._name = "v2_search"
  self._client = client
...
function V2SearchEntity:get_name()
  return self._name
end


function V2SearchEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return V2SearchEntity.new(self._client, opts)
end
...
function V2SearchEntity:mark_deleted()
  self._deleted = true
end


function V2SearchEntity:deleted()
  return true == self._deleted
end


function V2SearchEntity:data_set(args)
  if args ~= nil then
...
function V2SearchEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function V2SearchEntity:match_set(args)
  if args ~= nil then
...
function V2SearchEntity:match_get()
...
function V2SearchEntity:stream(action, args, callopts)
...
function V2SearchEntity:_run_op(ctx, post_done)
  local utility = self._utility
