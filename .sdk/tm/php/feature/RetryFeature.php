...
class FreeMusicRetryFeature extends FreeMusicBaseFeature
{
    ...
    public function init(FreeMusicContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (FreeMusicContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(FreeMusicContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(FreeMusicContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
