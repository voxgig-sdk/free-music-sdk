...
class FtCtrl extends FreeMusicControl
{
...
class FtClient
{
    ...
    private FreeMusicUtility $utility;

    public function __construct(FreeMusicUtility $utility, array $options)
    {
    ...
    public function get_utility(): FreeMusicUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (FreeMusicContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public FreeMusicContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (FreeMusicContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = FreeMusicConfig::make_feature($name);
    ...
    private static function buildUrl(FreeMusicSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof FreeMusicError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new FreeMusicResponse($fetched);
        }
    ...
    private function fail(FreeMusicContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (FreeMusicContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
