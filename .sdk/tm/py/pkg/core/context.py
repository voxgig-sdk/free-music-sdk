...
from projectname_sdk.core.control import FreeMusicControl
...
class FreeMusicContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        if isinstance(sp, FreeMusicSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, FreeMusicResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, FreeMusicResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def make_error(self, code, msg):
        return FreeMusicError(code, msg, self)
