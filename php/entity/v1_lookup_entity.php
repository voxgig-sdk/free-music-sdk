...
class V1LookupEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new V1LookupEntity($this->_client, $opts);
    }

    /**
     * @param V1Lookup|array $args V1Lookup data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = FreeMusicHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return V1Lookup|array The current V1Lookup data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of V1Lookup fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = FreeMusicHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of V1Lookup fields).
     */
    ...
    /**
     * Load a single V1Lookup.
     *
     * @param V1LookupLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed V1LookupLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return V1Lookup|array The loaded V1Lookup as an assoc-array at the
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = FreeMusicHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
    ...
    /**
     * List V1Lookup items matching the given filter.
     *
     * @param V1LookupListMatch|array|null $reqmatch Match filter (any subset
     *   of V1Lookup fields) as an assoc-array; V1LookupListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return V1Lookup[]|array A list of V1Lookup items as assoc-arrays at
