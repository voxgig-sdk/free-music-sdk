...
class FreeMusicMakeError
{
    public static function call(?FreeMusicContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new FreeMusicContext([], null);
        }
        $op = $ctx->op ?? new FreeMusicOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new FreeMusicResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof FreeMusicError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new FreeMusicError('', $msg, $ctx);
        ...
        if ($err instanceof FreeMusicError) {
