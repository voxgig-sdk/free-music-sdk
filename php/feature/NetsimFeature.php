...
class FreeMusicNetsimFeature extends FreeMusicBaseFeature
{
    ...
    private function _simulate(FreeMusicContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(FreeMusicContext $ctx, array $applied): void
    {
        $client = $this->client;
