...
class V1LookupEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = FreeMusicSDK::test(null, null);
        $ent = $testsdk->V1Lookup(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->V1Lookup(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->V1Lookup(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = v1_lookup_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "v1_lookup." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set FREE_MUSIC_TEST_V1_LOOKUP_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $v1_lookup_ref01_data = null;
        if (count($v1_lookup_ref01_data_raw) > 0) {
            $v1_lookup_ref01_data = Helpers::to_map($v1_lookup_ref01_data_raw[0][1]);
        }

        // LIST
        $v1_lookup_ref01_ent = $client->V1Lookup(null);
...
function v1_lookup_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/v1_lookup/V1LookupTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["v1_lookup01", "v1_lookup02", "v1_lookup03", "api_key01"] as $k) {
    ...
    $entid_env_raw = getenv("FREE_MUSIC_TEST_V1_LOOKUP_ENTID");
