...
class V1SearchDirectTest < Minitest::Test
  def test_direct_list_v1_search
    setup = v1_search_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-v1_search", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
    if setup[:live]
      ["api_key01"].each do |_live_key|
  ...
  def test_direct_load_v1_search
    setup = v1_search_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-v1_search", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
...
def v1_search_direct_setup(mockres)
