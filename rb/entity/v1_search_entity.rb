...
class V1SearchEntity
  ...
  def make
    opts = @_entopts.dup
    V1SearchEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = FreeMusicHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [V1Search, Hash] the current V1Search data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = FreeMusicHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of V1Search fields)
  def match_get
  ...
  #
  # @param reqmatch [V1SearchLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.V1Search.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [V1Search, Hash] the loaded V1Search; raises FreeMusicError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = FreeMusicHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [V1SearchListMatch, Hash, nil] match filter (any subset of
  #   V1Search fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<V1Search>, Array] the matching V1Search items; raises FreeMusicError on failure
