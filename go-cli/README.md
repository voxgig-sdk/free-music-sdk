...
boru-driven command-line client **and** interactive REPL for the FreeMusic
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/free-music-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/free-music-cli list v1_list
   ```
...
```sh
./free-music-cli load 1 v1_list          # scalar shorthand for {id:1}
./free-music-cli load '{id:1}' v1_list     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `free-music>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `v1_list`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/free-music-cli`. |
...
(`./free-music-cli <expr>`) and interactively (the REPL), and expressions compose the
