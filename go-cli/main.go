// Command FreeMusic-cli is a CLI + REPL for the FreeMusic SDK,
...
const prompt = "free-music"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "v1_list v1_lookup v1_search v2_list v2_lookup v2_search"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      FREE_MUSIC_APIKEY (api key), FREE_MUSIC_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: FREE_MUSIC_APIKEY carries the API key and
	// FREE_MUSIC_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("FREE_MUSIC_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("FREE_MUSIC_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewFreeMusicSDK(opts)
