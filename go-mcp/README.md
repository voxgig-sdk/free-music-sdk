...
[MCP](https://modelcontextprotocol.io) server exposing the FreeMusic SDK as
two agent tools — `free-music_list` and `free-music_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/free-music-mcp)
make build
...
claude mcp add --scope user free-music \
  -- /absolute/path/to/free-music-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./free-music-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// free-music_list: first page of records
{ "entity": "v1_list" }
{ "entity": "v1_list", "query": { } }

// free-music_load: one record by id
{ "entity": "v1_list", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/free-music-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user free-music \
     -- "$PWD"/dist/*/free-music-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `free-music_list` and `free-music_load` tools now appear
   in new sessions. Ask the agent to *"list v1_list using free-music"*
   and it calls `free-music_list` with `{"entity":"v1_list"}`.
...
```sh
export FREE_MUSIC_APIKEY=sk_live_xxx            # API key
export FREE_MUSIC_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `free-music_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `free-music_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `FREE_MUSIC_APIKEY` | API key sent with every request. |
| `FREE_MUSIC_BASE` | Optional override of the API base URL. |
...
```sh
./free-music-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"free-music_load","arguments":{"entity":"v1_list","query":{"id":1}}}}'
```
