...
func TestV1LookupEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.V1Lookup(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.V1Lookup(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		v1LookupRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.v1_lookup")))
...
func v1_lookupBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("FREE_MUSIC_TEST_V1_LOOKUP_ENTID")
	...
	idmapResolved := core.ToMapAny(env["FREE_MUSIC_TEST_V1_LOOKUP_ENTID"])
