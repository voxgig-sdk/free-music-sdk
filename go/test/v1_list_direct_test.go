...
func TestV1ListDirect(t *testing.T) {
	t.Run("direct-list-v1_list", func(t *testing.T) {
		setup := v1_listDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-v1_list", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-v1_list", func(t *testing.T) {
		setup := v1_listDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-v1_list", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type v1_listDirectSetupResult struct {
...
func v1_listDirectSetup(mockres any) *v1_listDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"FREE_MUSIC_TEST_V1_LIST_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["FREE_MUSIC_TEST_V1_LIST_ENTID"]; ok {
		...
		return &v1_listDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &v1_listDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
