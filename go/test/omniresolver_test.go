...
import (
	...
	omni "github.com/voxgig-sdk/free-music-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.FreeMusicError); is {
				out := map[string]any{
					"name":    "FreeMusicError",
...
func omniCtx(arg any, client *sdk.FreeMusicSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.FreeMusicSDK); is {
			client = live
