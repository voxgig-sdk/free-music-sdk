...
func (sdk *FreeMusicSDK) OptionsMap() map[string]any {
...
func (sdk *FreeMusicSDK) GetUtility() *Utility {
	return CopyUtility(sdk.utility)
}

func (sdk *FreeMusicSDK) GetRootCtx() *Context {
	return sdk.rootctx
}

func (sdk *FreeMusicSDK) Prepare(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *FreeMusicSDK) Direct(fetchargs map[string]any) (map[string]any, error) {
...
func (sdk *FreeMusicSDK) opAllowed(op string) bool {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return strings.Contains(allowOp, op)
}

func (sdk *FreeMusicSDK) opDenied(op string) map[string]any {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return map[string]any{
		"ok": false,
		"err": fmt.Errorf("FreeMusicSDK: %s: operation not allowed by"+
...
// gate by passing it.
func (sdk *FreeMusicSDK) rawRequest(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *FreeMusicSDK) Graphql(
	...
	if 0 < len(errors) {
		...
		res["ok"] = false
		res["err"] = fmt.Errorf("FreeMusicSDK: graphql: %s", msg)
		res["graphql"] = errors
	}
