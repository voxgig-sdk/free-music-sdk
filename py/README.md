...
The Python SDK for the FreeMusic API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.V1List(None).list(None, None)
if err:
    raise Exception(err)
...
```python
result, err = client.V1List(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.FreeMusic(None).load(
    {"id": "test01"}, None
)
