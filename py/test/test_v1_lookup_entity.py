...
class TestV1LookupEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.V1Lookup(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.V1Lookup(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set FREE_MUSIC_TEST_V1_LOOKUP_ENTID JSON to run live")
        client = setup["client"]
...
def _v1_lookup_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("FREE_MUSIC_TEST_V1_LOOKUP_ENTID"))
