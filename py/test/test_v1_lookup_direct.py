...
class TestV1LookupDirect:

    def test_should_direct_list_v1_lookup(self):
        setup = _v1_lookup_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-v1_lookup", "live" if setup["live"] else "unit")
        if _skip:
    ...
    def test_should_direct_load_v1_lookup(self):
        setup = _v1_lookup_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-v1_lookup", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
...
def _v1_lookup_direct_setup(mockres):
