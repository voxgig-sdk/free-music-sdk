...
class FreeMusicTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            FreeMusicTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = FreeMusicTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = FreeMusicTestRunner.getenv("FREE_MUSIC_TEST_LIVE")
        override = FreeMusicTestRunner.getenv("FREE_MUSIC_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = FreeMusicTestRunner.getenv(key)
        ...
        explain = FreeMusicTestRunner.getenv("FREE_MUSIC_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["FREE_MUSIC_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if FreeMusicTestRunner._test_control is not None:
            return FreeMusicTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                FreeMusicTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            FreeMusicTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = FreeMusicTestRunner.load_test_control()
...
def load_env_local():
    FreeMusicTestRunner.load_env_local()


def env_override(m):
    return FreeMusicTestRunner.env_override(m)


def entity_data(v):
    return FreeMusicTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return FreeMusicTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return FreeMusicTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return FreeMusicTestRunner.load_test_control()


def live_client_options():
    return FreeMusicTestRunner.live_client_options()


def live_delay_ms():
    return FreeMusicTestRunner.live_delay_ms()
