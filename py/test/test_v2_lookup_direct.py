...
class TestV2LookupDirect:

    def test_should_direct_load_v2_lookup(self):
        setup = _v2_lookup_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-v2_lookup", "live" if setup["live"] else "unit")
        if _skip:
...
def _v2_lookup_direct_setup(mockres):
